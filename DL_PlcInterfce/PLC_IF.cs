...
namespace DL_PlcInterfce
{
    public class PLC_IF
    {
        ...
        public UInt32 Open(string connectionParam)
        {
            ...
            try
            {
                ...
                if (_PLC == null)
                {
                    ...
                    else if (param[0].IndexOf("MELSEC") == 0)
                    {
                        _PLC = new Mitsubishi_PLC();
                        if (param.Length < 1)
                            rc = (UInt32)ErrorCodeList.INVALID_PARAMETER;
                    }
                    else
        ...
        /// <returns></returns>
        public UInt32 WaitVal(DeviceType deviceType, UInt32 addr, int value, int timeout)
        {
            UInt32 rc = 0;
            try
            {
                ...
                {
                    // 指定デバイス読込
                    rc = Read(deviceType, addr, ref readVal);
                    // 一致
                    if (STATUS_SUCCESS(rc) && readVal == value) break;
                    // タイムアウト確認
                    if (sw.ElapsedMilliseconds > timeout)
                        rc = (UInt32)ErrorCodeList.TIMEOUT;
