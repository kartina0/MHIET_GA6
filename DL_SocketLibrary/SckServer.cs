...
namespace DL_Socket
{
    ...
    public class SckServer
    {
        ...
        private void onClientConnect(IAsyncResult asynRes)
        {
            ...
            try
            {
                lock (_connectionLock)
                {
                    ...
                    if (sckServer != null)
                    {
                        // クライアントから接続されたので、通信用ソケットを取得
                        sckClient = sckServer.EndAccept(asynRes);
                        Console.WriteLine($"{sckClient.RemoteEndPoint.ToString()}");
                        _backupBuffer[sckClient.RemoteEndPoint.ToString()] = new byte[0];
                        WaitData(sckClient);
                        sckServer.BeginAccept(new AsyncCallback(onClientConnect), null);
        ...
        public UInt32 Send(byte[] byteArray)
        {
            UInt32 rc = 0;
            try
            {
                sckClient.Send(byteArray);
            }
            catch (Exception ex) 
            {
                rc = 1;
