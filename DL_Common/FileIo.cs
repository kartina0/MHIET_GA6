...
namespace DL_CommonLibrary
{
    ...
    public class FileIo
    {
        ...
        /// <param name="destFile"></param>
        /// <returns></returns>
        public static UInt32 MoveFile(string srcFile, string destFile)
        {
            UInt32 rs = 0;
            int retry = 0;
            try
            {
                if (!ExistFile(srcFile)) rs = (UInt32)ErrorCodeList.FILE_NOT_FOUND;

                if (rs == 0)
                {
                    for (int i = 0; i < 5; i++)// ﾘﾄﾗｲ
                    {
                        rs = CopyFile(srcFile, destFile);
                        if (rs == 0) break;
                    }
                    Thread.Sleep(10);
                }
        ...
        public static bool DeleteFileEx(string dir, string baseFileName, DateTime time)
        {
            ...
            try
            {
                ...
                if (ext.Length > 0 && ext[0] == '.') ext = ext.Remove(0, 1);
