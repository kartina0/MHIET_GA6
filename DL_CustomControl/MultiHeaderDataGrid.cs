...
namespace DL_CustomControl
{
    public partial class MultiHeaderDataGrid : DataGridView
    {
        ...
        /// <summary>
        /// @@20221204
        /// ファイルからヘッダーを表示する際の行番号を指定
        /// </summary>
        private int _fileHeaderLineNo = 0;
        ...
        [Category("Data"),
         Description("ヘッダー文字列をファイル場合の行番号"),
         DefaultValue(""),
        ]
        public int HeaderTextLineNo
        {
            get { return _fileHeaderLineNo; }
            set
            {
                _fileHeaderLineNo = value;
            }
        }
        ...
        private void ReadCsvFileData(string[] files)
        {
            try
            {
                ...
                for (int i = 0; i < files.Length; i++)
                {
                    ...
                    {
                        // 日本語指定で開く
                        using (TextReader tr = new StreamReader(fs, System.Text.Encoding.GetEncoding("shift-jis")))
                        {
                            buf = tr.ReadToEnd();
                    ...
                    else
                    {
                        if (_fixedHeaderStyle)
                            ...
                            readEndLine = _fileHeaderLineNo + 1;    // ヘッダー行は読み込まないようにするため 1 にしておく
                    }
                    // 登録する行数を確認
                    ...
                    if (_firstFileLoad)
                    {
                        ...
                        {
                            _readCsvHeater = lines[_fileHeaderLineNo].Split(',');
                        }
#else
                        if (!_fixedHeaderStyle && i == 0)
                        {
        ...
        private void UpdateDataGrid()
        {
            ...
            // 全行のデータを書き込む
            var l = _readCsvData.OrderByDescending(x => x[0]);
            if (this.Columns.Count > 0)
            {
                ...
                {
                    this.Rows.Add(ll);
                    //this.Rows[this.RowCount - 2].DefaultCellStyle.Font = new Font("メイリオ", 12);
                    //this.Rows[this.RowCount - 2].DefaultCellStyle.BackColor = Color.MistyRose;
                    //this.Rows[this.RowCount - 2].Height = 30;
                    
                    //this.Rows[this.RowCount - 1].DefaultCellStyle.Font = new Font("メイリオ", 12);
                    //this.Rows[this.RowCount - 1].DefaultCellStyle.BackColor = Color.MistyRose;
                    //this.Rows[this.RowCount - 1].Height = 30;

                    //DateTime today_date = DateTime.Today;
                    //Color c = Color.Black;
                    //if (DateTime.Parse(ll[0]) > today_date)
                    //    this.Rows[this.RowCount - 2].DefaultCellStyle.ForeColor = Color.Red;
                }
            }
        }
